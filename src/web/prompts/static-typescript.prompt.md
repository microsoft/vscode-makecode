Your goal is to complete the users request within the context of a MakeCode program, which uses the PXT compiler

* When possible, prioritize using APIs that are exposed as blocks; these can be identified via the `//% block` annotations on their typing.
* If a value's name is prefixed with `_`, we elide it from the completions in the typescript editor; it should not be included unless it is particularly necessary.
* If a value is marked `//% deprecated`, it should be deprioritized; there is likely an alternative that is *not* deprecated.
* Do not hallucinate functions, methods, or packages outside the active MakeCode project. The project may be a normal folder or an imported/share-backed `mkcdfs:` folder exposed by the VS Code MakeCode extension. Start from the nearest `pxt.json`, then inspect project `.ts` files and the nearest `/pxt_modules` folder when present. Target base libraries and installed extension dependencies are available under `/pxt_modules/<package>/` as `.ts` / `.d.ts` files, with package membership described by each package's `pxt.json`.
* If `/pxt_modules` is missing or incomplete, ask the user to run **MakeCode: Install Project Dependencies** or provide the target/dependency context before using APIs that are not visible. The APIs and packages available are limited, and do not include arbitrary node or browser packages. Things like `document.createElement` will not be available.
* When adding a new file, it should be added properly to `pxt.json`
* When creating assets as part of the users prompt, use tagged templates like below instead of suggesting inline literals to allow the user to edit them and make them their own with the vscode-makecode extension:
* Blocks should have a help attribute, defining where the help documentation for the block is. Format is `//% help={location}`. If it is just a path, it should be found under a docs folder in pxt, pxt-common-packages, or the target repository. If the location is prefixed with `github:`, it will be stored in the extension itself -- e.g. `//% help=github:pxt-status-bars/docs/create` is found in the `pxt-status-bars` extension itself, in `docs/create.md`.
...
Do that instead of inline assets like 
...
Use descriptive asset names so that the user can have an expectation of what they are when they go to create them.

* Do not edit assets.ts; leave that to the user to do with the vscode extension.
* The PXT compiler uses static typescript as defined in the following documentation from microsoft/pxt/docs/language.md:
