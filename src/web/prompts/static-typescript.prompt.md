...
* Do not hallucinate functions, methods, or packages outside what is listed under the nearest `/pxt_modules` folder; the api and packages available are limited, and do not include further node or browser packages. Things like `document.createElement` will not be available.
